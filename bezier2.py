...
for i in range(7):
	j = 0.0;
	prev = bez(points[i*9],points[i*9 + 3],points[i*9 + 6],points[i*9 + 9],0.0)
	while(j <= 1.0):
		res = bez(points[i*9],points[i*9 + 3],points[i*9 + 6],points[i*9 + 9],j)
		win.create_line(prev[0],prev[1] - 100,res[0],res[1] - 100,fill="black")
		j = j + dj
		prev = res
...
for i in range(21):
	...
	while(j <= 1.0):
		...
		resB = bez((0.0,100.0,0.0),(250.0,50.0,0.0),(750.0,50.0,0.0),(1000.0,100.0,0.0),(i + j)/21.0)
